...
If ( $Notify.Count -gt 1 ) {
    ...
    If ($result.Verified) { $send = $true ; ./Send-Message -Content $(Format-TimeLog "Service-EncodeMp4toWebm Started.") -Username $result.Username -Webhookuri $result.Webhookuri } Else { "Could not verify '{0}','{1}'." -f $Notify[0], $Notify[1] ; Exit }
...
function Run {
    ...
    ForEach ( $folder In $(Get-Childitem -LiteralPath $Folder) ) {
        ...
        function Replace-Extension {
            ...
            return $($S -replace "\..*$",".webm")

        }

        [Object[]]$items = Get-ChildItem -LiteralPath $folder.FullName | Where { $_.Name -imatch "^.*\.mp4" }

        If ( $items -ne $null ) {

            ForEach ( $x In $items ) {

                [String]$temp = Replace-Extension $x.FullName | Split-Path -Leaf

                [System.IO.FileSystemInfo]$webmfiles = $(Get-ChildItem -LiteralPath $folder.FullName) | Where { $_ -imatch "^.*$([Regex]::Escape($temp))" }

                If ( $webmfiles -eq "" -or $webmfiles -eq $null ) {
                    ...
                    If ($send) { ./Send-Message -Content $temp -Username $result.Username -Webhookuri $result.Webhookuri }
                    ...
                    HandbrakeCLI --preset-import-gui -Z "Creator 1080p30 webm" -i $x.FullName -o $(Replace-Extension $x.FullName) 2>> $log

                    [System.DateTime]$endtime = Get-Date

                    [String]$finished = "Finished in {0}." -f $(Format-Time $(New-TimeSpan -Start $starttime -End $endtime))
                    ...
                    [String]$temp = Format-TimeLog $("Sleeping for '{0}' minutes." -f $minutes)
...
While ($true) { Run ; Start-Sleep -Seconds $($minutes*60)}
